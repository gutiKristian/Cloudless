...
class TestUtils:
    ...
    def test_ndvi(self):
        ...
        assert np.array_equal(ndvi(first, second), second.astype(float))
        assert np.allclose(ndvi(second, third), np.array([0, 1/3, 0.5, 3/5, 2/3, 5/7], dtype=float), atol=0.0001)
